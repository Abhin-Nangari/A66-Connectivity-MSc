...
#
# Purpose :
#   1. Pre-flight validation — verify all Script 3 outputs, Julia, Circuitscape
#   2. Build focal node raster — 15 largest woodland patches, unique IDs 1-15
#      Uses additive integer method — guaranteed unique IDs, fully reproducible
...
#
# ASCII export method:
#   arcpy.RasterToASCII consistently stalls on Parallels Desktop due to
#   file locking behaviour with UNC paths. This script uses
#   arcpy.RasterToNumPyArray to read raster data into memory and writes
#   the ASCII file directly using Python file I/O — fully reliable and
#   produces identical output to RasterToASCII.
#
# Focal node design:
#   1107 woodland patches identified in 2km buffer.
#   83 patches >= 1 hectare (2500 pixels at 2m resolution).
#   15 largest patches selected as focal nodes.
#   Pre-verified patch IDs and sizes from diagnostic run.
...
#   Script prompts user at the correct moment — do not close before prompted.
#
...
# Pre-verified focal node patch IDs and sizes from diagnostic run
...
def export_ascii_numpy(in_tif, out_asc, name, nodata=-9999):
    ...
    Produces identical output — header + space-delimited row data.
    """
...
# ---------------------------------------------------------------------------
# Additive integer method — guaranteed unique IDs 1-15.
# Computes woodland mask, region group, then assigns sequential IDs.
...
# ---------------------------------------------------------------------------
# Uses numpy RasterToNumPyArray method — reliable on Parallels UNC paths.
# arcpy.RasterToASCII avoided — known to stall on this hardware configuration.

print("\n--- STAGE 2: Export ASCII Files (numpy method) ---")

nodes_asc = os.path.join(CS_DIR, "focal_nodes.asc")
export_ascii_numpy(nodes_path, nodes_asc, "focal_nodes")
...
# ---------------------------------------------------------------------------
# all-to-one mode: each node connected to ground in turn, all others
# act as current sources. Produces cumulative current map.
# Forward slashes used throughout — required for Julia/Circuitscape.
...
print(f"  All scenarios finished: "
      f"{datetime.datetime.now().strftime('%H:%M:%S')}")
print(f"  Successful: {len(curmap_paths)} of 4")
...
# wildlife corridor on the A66 upgrade — alignment chosen to avoid it.
...
for f in sorted(os.listdir(CS_DIR)):
    if not f.startswith(".") and f != "Thumbs.db":
        size_mb = os.path.getsize(
            os.path.join(CS_DIR, f)) / 1024 / 1024
...
print("  [ ] S1 vs S4 — similar spatial pattern (high stability)")
