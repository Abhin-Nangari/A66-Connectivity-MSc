...
#      ArcGIS Pro stays open throughout — subprocess runs independently
#   3. Verify Circuitscape ASCII outputs exist and are correctly sized
...
#
# Prerequisites — Script 4a must be complete before running this script:
...
# Ensure output folders exist — Circuitscape requires the output directory
# to exist before running. Created here in case they were deleted after 4a.
...
# ---------------------------------------------------------------------------
# STAGE 3 — IMPORT CURRENT DENSITY RASTERS INTO ARCGIS PRO
# ---------------------------------------------------------------------------
...
# National Highways TR010062 (2019) Citizen Space consultation explicitly
# states Option E was selected because it "will also not impact on the
# wildlife corridor on the disused railway line" — the only formally named
# wildlife corridor on the entire A66 upgrade.
# Validation: sample current density at 5 railway points and 3 A66 road
# points. Compute mean values and railway-to-road ratio per scenario.
# Expected: railway mean > road mean across all scenarios.
# Citation: National Highways (2019) NH Citizen Space TR010062
#           SI 2024/360 Schedule 7 Part 3
...
for scenario_name, tif_path in curmap_tifs.items():
    ...
    if railway_vals and road_vals:
        ...
        if rail_mean > road_mean:
            print(f"  [OK] Railway > A66 road — validates as corridor.")
        else:
