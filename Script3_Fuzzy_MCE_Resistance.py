...
#   4. Weighted overlay — 4 sensitivity scenarios:
#        S1 — Primary    : LCM=0.40  VRI=0.30  Slope=0.20  CHM=0.10
#        S2 — LCM +20pp  : LCM=0.48  VRI=0.26  Slope=0.18  CHM=0.08
#        S3 — LCM -20pp  : LCM=0.32  VRI=0.34  Slope=0.22  CHM=0.12
#        S4 — Equal      : LCM=0.25  VRI=0.25  Slope=0.25  CHM=0.25
#   5. Pre-run weight validation — all weights verified to sum to 1.0
#      and produce meaningfully different surfaces before saving
#   6. Spatial stability raster — pixel-wise range across all 4 scenarios
#   7. Full verification at every stage — band count, extent, statistics
#
# Sensitivity scenario design rationale:
#   LCM carries the highest weight (0.40) and highest uncertainty in the
#   primary scenario. S2 and S3 vary the LCM weight by ±20 percentage
#   points, redistributing the difference proportionally across the
#   remaining three weights. This tests sensitivity to the most influential
#   and most uncertain parameter — the standard approach in published
#   resistance surface literature (Zeller et al. 2012; Liu et al. 2018;
#   Dutta et al. 2022). S4 tests the null hypothesis that equal weighting
#   produces equivalent corridors.
#
#   S1: LCM=0.40  VRI=0.30  Slope=0.20  CHM=0.10  Sum=1.00
#   S2: LCM=0.48  VRI=0.26  Slope=0.18  CHM=0.08  Sum=1.00
#       (+8pp LCM; remainder redistributed proportionally: VRI*0.867,
#        Slope*0.867, CHM*0.867)
#   S3: LCM=0.32  VRI=0.34  Slope=0.22  CHM=0.12  Sum=1.00
#       (-8pp LCM; remainder redistributed proportionally: VRI*1.133,
#        Slope*1.133, CHM*1.133)
#   S4: LCM=0.25  VRI=0.25  Slope=0.25  CHM=0.25  Sum=1.00
#
# Memory management:
#   Each stage saves to disk and releases memory before proceeding.
#   Each scenario processed independently — loaded fresh, saved, released.
...
#   Sensitivity design — Dutta et al. (2022) Landscape Ecology 37:2195-2224
#
...
for name, (path, exp_min, exp_max) in inputs.items():
    ...
    if band_ok and ext_ok and range_ok:
        ...
        if not ext_ok:
            print(f"    Extent mismatch. Rerun Script 2.")
        if not range_ok:
            print(f"    Range {r_min:.2f}-{r_max:.2f} outside expected.")
...
# ---------------------------------------------------------------------------
# STAGE 1 — DEFINE AND VALIDATE SENSITIVITY SCENARIOS
# ---------------------------------------------------------------------------
# Weights defined explicitly — no algorithmic generation.
# Each scenario verified to sum to 1.0 before any processing begins.
# S2 and S3 verified to produce different weights from S1.
# Design: vary LCM weight by ±8 percentage points (±20% of 0.40),
# redistribute difference proportionally across remaining three weights.
# Citation: Dutta et al. (2022) Landscape Ecology 37:2195-2224

print("\n--- STAGE 1: Define and Validate Sensitivity Scenarios ---")

# Weights: (LCM, VRI, Slope, CHM)
SCENARIOS = [
    ("S1_primary",
     (0.40, 0.30, 0.20, 0.10),
     "Primary weights — Zeller et al. (2012)"),

    ("S2_LCM_plus20pp",
     (0.48, 0.26, 0.18, 0.08),
     "LCM +20pp — remainder redistributed proportionally"),
    ...
    ("S4_equal",
     (0.25, 0.25, 0.25, 0.25),
     "Equal weights — null model sensitivity test"),
]
...
for name, weights, desc in SCENARIOS:
    ...
    sum_ok = abs(weight_sum - 1.0) <= 0.0001
    print(f"  {name}:")
    print(f"    LCM={w_lcm}  VRI={w_vri}  Slope={w_slope}  CHM={w_chm}  Sum={weight_sum}")
    if sum_ok:
        print(f"    [OK] Weights sum to 1.0")
    else:
        print(f"    [ABORT] Weights do not sum to 1.0")
        scenario_ok = False

# Verify S2 and S3 differ from S1
s1_weights = SCENARIOS[0][1]
s2_weights = SCENARIOS[1][1]
s3_weights = SCENARIOS[2][1]

if s2_weights == s1_weights:
    print("\n  [ABORT] S2 weights identical to S1 — sensitivity analysis invalid.")
    scenario_ok = False
else:
    print(f"\n  [OK] S2 differs from S1: LCM {s1_weights[0]} → {s2_weights[0]}")

if s3_weights == s1_weights:
    print("  [ABORT] S3 weights identical to S1 — sensitivity analysis invalid.")
    scenario_ok = False
else:
    print(f"  [OK] S3 differs from S1: LCM {s1_weights[0]} → {s3_weights[0]}")

if not scenario_ok:
    raise RuntimeError(
        "Scenario validation failed. Fix weights before rerunning Script 3.")

print("\n  [OK] All scenarios validated — weights correct and distinct.")

# ---------------------------------------------------------------------------
# STAGE 2 — FUZZY MEMBERSHIP FUNCTIONS
# ---------------------------------------------------------------------------
# All layers already normalised 0-1 in Script 2.
# Fuzzy functions convert each to resistance score 0-1
...
# ---------------------------------------------------------------------------
# STAGE 3 — LCM RESISTANCE RECLASSIFICATION
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# STAGE 4 — WEIGHTED OVERLAY — 4 SENSITIVITY SCENARIOS
# ---------------------------------------------------------------------------
...
# All weights verified in Stage 1 to sum to 1.0 and be mutually distinct.
...
for name, weights, desc in SCENARIOS:
    ...
    r_min, r_max, r_mean, r_std = verify(
        out_path, f"Resistance_{name}",
        exp_min=0.0, exp_max=1.0, exp_bands=1)
    scenario_means[name] = r_mean
...
# Post-overlay validation — confirm all scenarios produce different surfaces
print("\n  Post-overlay distinctness check:")
means = list(scenario_means.values())
names = list(scenario_means.keys())
all_distinct = True
for i in range(len(means)):
    for j in range(i+1, len(means)):
        if abs(means[i] - means[j]) < 0.0001:
            print(f"  [WARNING] {names[i]} and {names[j]} have identical means — check weights.")
            all_distinct = False
        else:
            print(f"  [OK] {names[i]} vs {names[j]}: mean difference = {abs(means[i]-means[j]):.4f}")

if all_distinct:
    print("  [OK] All scenarios produce distinct resistance surfaces.")

# ---------------------------------------------------------------------------
# STAGE 5 — SPATIAL STABILITY CHECK
# ---------------------------------------------------------------------------
...
all_outputs = {
    ...
    "Resistance_S2_LCM_plus20pp.tif"   : resistance_paths[1],
    "Resistance_S3_LCM_minus20pp.tif"  : resistance_paths[2],
...
print()
print("SCENARIO MEAN RESISTANCE VALUES:")
...
print("  [ ] S2 visually brighter than S1 — higher LCM weight = more contrast")
print("  [ ] S3 visually softer than S1 — lower LCM weight = less contrast")
...
print("  S1: LCM=0.40  VRI=0.30  Slope=0.20  CHM=0.10  (primary)")
print("  S2: LCM=0.48  VRI=0.26  Slope=0.18  CHM=0.08  (LCM +20pp)")
print("  S3: LCM=0.32  VRI=0.34  Slope=0.22  CHM=0.12  (LCM -20pp)")
print("  S4: LCM=0.25  VRI=0.25  Slope=0.25  CHM=0.25  (equal weights)")
print("  All scenarios: distinct weights, verified sum=1.0")
